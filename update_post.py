...
class UpdatePost(SessionBase):
    ...
    def based_on_praise_and_time(self):
        ...
        # 获取所有帖子的postid以及创建时间
        _sql_get_all_id_and_creatTime = "SELECT p.post_id, p.customer_id, p.topic_circle_id,c.count, p.create_time FROM px_post p LEFT JOIN (SELECT l.post_id,COUNT(post_id) count FROM px_post_like l GROUP BY l.post_id) c ON p.post_id = c.post_id WHERE p.check_status='1' and p.del_flag = '0' and p.top_status = '0'"
        mycursor.execute(_sql_get_all_id_and_creatTime)
        myresult = mycursor.fetchall()
        # 帖子id 所属圈子id 点赞量 帖子创建时间
        df_id_and_time = pd.DataFrame(myresult, columns=['post_id', 'customer_id', 'topic_circle_id', 'like', 'create_time'])
        ...
        res['value'] = res[['diff', 'like']].apply(lambda x: (x['like'] * 50 + 10) / (math.exp(0.144 * x['diff'])), axis=1)
        ...
        for index, row in res.iterrows():
            post_customer_tuple.append((row['post_id'], row['customer_id']))

        for idx, item in enumerate(post_customer_tuple):
            if buffer != [] and len(result) >= 3:
                for i in range(len(buffer)):
                    if len({buffer[i][1], result[-1][1], result[-2][1], result[-3][1]}) == 4:
                        result.append(buffer.pop(i))
                        break
            if item[1] not in position.keys():
                position[item[1]] = idx
                result.append(item)
                continue
            buffer.append(item)
        random.shuffle(buffer)
        rank_list = [x[0] for x in result] + [y[0] for y in buffer]
        # print(rank_list)
